...
def pascal_triangle(num):
    with open('pascal_triangle.txt', 'w', encoding='utf-8') as pt:
        ...
        for i in range(num):
            ...
            for j in range(i + 1):
                line.append(
                    f'{bk(i, j):^{longest}d}')  # центрируем числа по длине наибольшего числа и собираем в список
            print(''.join(line).center(longest * num),
                  file=pt)  # преобразуем список в строку и центрируем её по длине наибольшей строки


with open('num.txt', 'r') as file:
    number = int(file.read().strip())  # считываем число из первого файла
